...
**定義**: 全エントリのうち、[事後入力](#事後入力)・[事後修正](#事後修正)（`add` / `edit`）によって
生成・書き換えされたエントリが占める割合

**説明**:
PRD のセカンダリー KPI（10% 以下）。リアルタイム計測が習慣として定着しているかを測る指標であり、
この値が高い場合は「計測を忘れて後から思い出して埋めている」状態を意味する。

集計は [Entry](#entry) の `source` フィールド（`'realtime'` / `'manual'`）を月次で数えて行う。
`source` は **P0 の段階から付与する**。`add` / `edit` は P1 の機能だが、後からフィールドを追加すると
P0 期間のエントリが経路不明となり、分母（全エントリ）を遡って集計できなくなるため。

測定開始は導入 2 ヶ月後（PRD「セカンダリーKPI」参照）。

**関連用語**: [事後入力](#事後入力)、[事後修正](#事後修正)、[Entry](#entry)

**英語表記**: Manual Entry Rate
...
- `note`: [備考](#備考)（設定時のみ存在）
- `source`: 作成経路。`'realtime'`（`start` / `stop` 由来。起動時の[復帰フロー](#復帰フロー)による確定を含む）
  または `'manual'`（`add` / `edit`（P1）由来）。PRD のセカンダリー KPI「[事後修正率](#事後修正率)」の集計に用いる
...
**主要フィールド**:
- `version`: スキーマバージョン（現在は `1` 固定）。将来のスキーマ移行の判定に用いる
...
| 2026-08-17 | `Entry` に `source` フィールドを追記、KPI 用語「事後修正率」を新規追加、`CurrentTimer` に `version` フィールドを追記、索引に「最近使った順」「事後修正率」を追加、更新日を最新化（用語集レビュー対応） |
