...
スコープは 2 系統ある。**`src/services/` 配下の変更は機能名を使い、それ以外はレイヤー名を使う**。
サービスレイヤーだけ機能名にするのは、`services/` が機能ごとに 1 クラス対応しており
（`TimerService` / `ProjectService` / …）、レイヤー名では「どの機能を触ったか」が失われるため。

| 系統 | スコープ | 対応 |
|------|---------|------|
| 機能（`src/services/`） | `timer` / `project` / `report` / `export` | 各サービスクラス |
| レイヤー | `repl` / `domain` / `stores` / `formatters` / `validators` / `types` | `src/` の同名ディレクトリ |
| その他 | `config` | `package.json` / `tsconfig.json` / lint・format 設定 / Git フック |

**複数のスコープにまたがる場合**は、変更の主目的にあたる 1 つを選ぶ。
機能追加に伴って複数レイヤーを触ったのであれば機能スコープを使う
（例: 日跨ぎ分割の実装で `domain/` と `stores/` を触っても `feat(timer)`）。
純粋にレイヤー内の整理であればレイヤースコープを使う
（例: `formatDuration.ts` 単独の変更は `refactor(formatters)`）。
どれにも当てはまらない場合はスコープを省略してよい。
...
**レビュアーを確保できない場合**: 開発体制が 1 人のときは 3 を省略し、
セルフレビューと自動チェックの通過をもって承認とする。その際は PR 本文に
「レビュアー不在のためセルフレビューで承認」と明記し、省略した事実を履歴に残す。
1 と 2 は省略しない。特にセルフレビューは、レビュアーがいない体制では
「コードレビュー基準」のチェックリストを自分で 1 項目ずつ確認する作業に置き換わる。
...
```
     ...
     ╱     ╲
    ╱───────╲  統合 (中程度)
   ╱         ╲ 6 本: ライフサイクル / 日跨ぎ永続化 / 破損データ / 原子性 / 権限 / 復帰ログ
  ╱───────────╲
 ╱             ╲ ユニット (多数・高速)
...
- [ ] `devDependencies` を追加・更新した場合、下記「依存追加時のレビュー」を満たしているか
...
`dependencies`（実行時依存）の追加は必須ルール 6 で禁止されている。
`devDependencies` は追加できるが、以下を PR 本文で説明し、レビューで確認する。

- [ ] **必要性**: 標準ライブラリ・既存の devDependencies で代替できない理由が書かれているか
- [ ] **バージョン指定**: `docs/architecture.md` の「バージョン管理方針」表に沿ったレンジか
- [ ] **バージョン整合**: 関連パッケージ群を同一マイナーに揃える制約に違反していないか
      （特に `vitest` / `@vitest/coverage-v8` / `@vitest/ui` は内部 API を共有しており、ずれると起動しない）
- [ ] **実行時への漏れ出しがないか**: `src/` から import していないか。
      import が必要なら実行時依存であり、`devDependencies` に置くのは誤りである
- [ ] **ドキュメント更新**: 新しいツールを増やした場合、`docs/architecture.md` の表と
      本ガイドラインの「開発中に使うコマンド」を更新したか
...
| `npx vitest run tests/unit/domain/splitByDay.test.ts` | 指定した 1 ファイルのみ実行（フルスイートを待たずに回す） |
| `npx vitest run -t "日跨ぎ"` | テスト名に部分一致するケースのみ実行 |
...
`.husky/pre-commit` で以下の 2 つが順に実行される（既存設定）。

```
1. npx lint-staged     # *.{ts,tsx} → eslint --fix → prettier --write
2. npm run typecheck   # ステージング差分に限らずプロジェクト全体を型検査
```

**PR セルフチェックで `npm run typecheck` を重複して要求している理由**:
pre-commit は `--no-verify` で迂回でき、リベースやマージの結果として
「各コミット単体では通るが統合結果では型が壊れる」状態も起こり得る。
PR 時点でもう一度全体を検査することで、その両方を塞いでいる。
...
`docs/` 配下のドキュメントを更新する際は、以下をセットで行う。

1. **本文の修正**
2. **変更履歴への行追加**（日付と変更内容。どのレビュー・作業に対応するかを併記する）
3. **ヘッダーの「更新日」を、いま追加した変更履歴の日付に合わせる**

3 を忘れると、変更履歴には最新の更新が並んでいるのにヘッダーは初版のまま、という状態になる。
読者はまずヘッダーの日付でドキュメントの鮮度を判断するため、古い日付が残っていると
「このドキュメントは更新されていない＝内容が信用できない」と誤読される。

**エンティティのフィールドを追加・変更した場合**は、影響範囲が 1 ファイルに閉じない。
`docs/functional-design.md` のインターフェース定義と `docs/glossary.md` のデータモデル用語は
同じものを二箇所に書いているため、**必ず両方を更新する**。
...
|------|---------|
| 2026-08-12 | 初版作成 |
| 2026-08-17 | 統合テストを 5 本 → 6 本（復帰ログを追加）、pre-commit の実行内容に `npm run typecheck` を追記、コミットスコープを機能系統／レイヤー系統に整理して `formatters` / `validators` を追加・`store` → `stores` に修正、「依存追加時のレビュー」節を追加、単一テストファイル実行コマンドを追加、レビュアー不在時の代替フローを追加、本変更履歴を追加（開発ガイドラインレビュー対応） |
| 2026-08-17 | 「ドキュメント更新時のルール」節を追加（変更履歴と更新日の同時更新、エンティティ変更時の二箇所更新）、ヘッダーに「更新日」を追加（用語集レビュー対応） |
